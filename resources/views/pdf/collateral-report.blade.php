...
<head>
    ...
    <title>{{ __('messages.collateral_of', ['vehicle' => $collateral->vehicle_id]) }}</title>
    <style>
        body {
...
<body>
    <div class="container">
        ...
        <div class="title">{{ __('messages.collateral_of', ['vehicle' => $collateral->vehicle_id]) }}</div>

        <table style="width: 100%; border: none;">
            <tr>
                <td style="width: 33%; padding: 5px; border: none;"><strong>{{ __('Vehicle') }}:</strong>
                    <br>{{ $collateral->vehicle_id }}
                </td>
                <td style="width: 33%; padding: 5px; border: none;"><strong>{{ __('User type') }}:</strong>
                    <br>{{ __($collateral->user_type) }}
                </td>
                <td style="width: 33%; padding: 5px; border: none;"><strong>{{ __('Full Name') }}:</strong>
                    <br>{{ $collateral->driver_partner_name }}
                </td>
            </tr>
            <tr>
                <td style="padding: 5px; border: none;"><strong>{{ __('Description') }}:</strong>
                    <br>{{ $collateral->description }}
                </td>
                <td style="padding: 5px; border: none;"><strong>{{ __('Date') }}:</strong>
                    <br>{{ $collateral->start_date }}
                </td>
                <td style="padding: 5px; border: none;"><strong>{{ __('Payment frequency') }}:</strong>
                    <br>{{ __($collateral->payment_frequency) }}
                </td>
            </tr>
            <tr>
              <td style="padding: 5px; border: none;"><strong>{{ __('Number of Installments') }}:</strong>
                <br>
                {{ $collateral->interest_payment_method === 'separate' ? $collateral->instalments + 1 : $collateral->instalments }}
                {{ __('Fees') }}
            </td>

                <td style="padding: 5px; border: none;"><strong>{{ __('Amount') }}:</strong> {{ $currency }}.
                    <br>{{ $collateral->amount }}
                </td>
                <td style="padding: 5px; border: none;"><strong>{{ __('Status') }}:</strong>
                  <br>{{ __($collateral->status) }}
              </td>
            </tr>
        </table>
        ...
        <table>
            ...
            <tbody>
                ...
                @empty
                    <tr>
                        <td colspan="5" style="text-align: center;">{{ __('No fees were found for this collateral.') }}
                        </td>
                    </tr>
                @endforelse
            </tbody>
