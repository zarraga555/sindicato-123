...
<div class="container">
    ...
    <div class="report-title">{{__('Daily income and expense report')}}</div>
    ...
    <table>
        ...
        <tr>
            <td>{{ $item->vehicle_id ? str_pad($item->vehicle_id,3,0, STR_PAD_LEFT) : 'S/M'}}</td>
            <td>{{ $item->banks ? $item->banks->bank_name : 'S/B' }}</td>
            <td>{{ $item->itemsCashFlow->name?? __($item->type_transaction) }}</td>
            <td>{{ __($item->transaction_type_income_expense) }}</td>
            <td>{{ __($item->roadmap_series) }}</td>
    ...
    <div class="summary">
        ...
        <p><strong>{{ __('Total Expenses') }}:</strong> Bs. {{ number_format($totalExpenses, 2) }}</p>
        <p><strong>{{ __('Net Total') }}:</strong> Bs. {{ number_format($netTotal, 2) }}</p>
    </div>
    ...
    <div class="summarytop">
        ...
        @if ($items->where('transaction_type_income_expense', 'income')->isNotEmpty())
            ...
            @foreach($items->where('transaction_type_income_expense', 'income')->whereNotNull('items_id')->groupBy('items_id') as $items_id => $group)
                @php
                    $totalIncome = $group->sum('amount');
                @endphp
                <p class="indented"><strong>&nbsp;&nbsp;&nbsp;&nbsp; * {{ $group->first()->itemsCashFlow->name ?? __('Unknown') }}:</strong> Bs. {{ number_format($totalIncome, 2) }}</p>
            @endforeach
            ...
            @if ($initialAccountAmount > 0)
                <p class="indented"><strong>&nbsp;&nbsp;&nbsp;&nbsp; * {{ __('Initial Account Amount') }}:</strong> Bs. {{ number_format($initialAccountAmount, 2) }}</p>
            @endif
            ...
            @endphp
            @if ($paymentOfQuota > 0)
                <p class="indented"><strong>&nbsp;&nbsp;&nbsp;&nbsp; * {{ __('Payment of Quota') }}:</strong> Bs. {{ number_format($paymentOfQuota, 2) }}</p>
            @endif
        ...
        @if($items->where('transaction_type_income_expense', 'expense')->isNotEmpty())
            ...
            @foreach($items->where('transaction_type_income_expense', 'expense')->whereNotNull('items_id')->groupBy('items_id') as $items_id => $group)
                @php
                    $totalIncome = $group->sum('amount');
                @endphp
                <p class="indented"><strong>&nbsp;&nbsp;&nbsp;&nbsp; * {{ $group->first()->itemsCashFlow->name ?? __('Unknown') }}:</strong> Bs. {{ number_format($totalIncome, 2) }}</p>
            ...
            @endphp
            @if ($initialAccountAmount > 0)
                <p class="indented"><strong>&nbsp;&nbsp;&nbsp;&nbsp; * {{ __('Loan') }}:</strong> Bs. {{ number_format($initialAccountAmount, 2) }}</p>
            @endif
        @endif
    </div>
