...
<head>
    ...
    <style>
        ...
        .report-date p {
            ...
            display: block;
            /* Asegura que el <p> es un elemento bloque */
            text-align: center;
            /* Aunque ya está en .report-date, asegúrate de que el <p> también lo tenga */
        }
            ...
            <tbody>
                @forelse($items as $item)
                    <tr>
                        ...
                        <td>{{ $item->vehicle_id ? str_pad($item->vehicle_id, 3, 0, STR_PAD_LEFT) : 'Sin movil asociado' }}
                        </td>
                        ...
                        <td>{{ $item->banks ? $item->banks->currency_type . '.' : '' }}
        ...
        <div class="summarytop">
            ...
            @if ($items->where('transaction_type_income_expense', 'income')->isNotEmpty())
                ...
                @foreach ($items->where('transaction_type_income_expense', 'income')->whereNotNull('items_id')->groupBy('items_id') as $items_id => $group)
                    ...
                    @endphp
                    <p class="indented"><strong>&nbsp;&nbsp;&nbsp;&nbsp; *
                            {{ $group->first()->itemsCashFlow->name ?? __('Unknown') }}:</strong> Bs.
                        {{ number_format($totalIncome, 2) }}</p>
                @endforeach

                {{-- 2️⃣ Incluir "initial account amount" --}}
                @php
                    $initialAccountAmount = $items
                        ->where('transaction_type_income_expense', 'income')
                        ->where('type_transaction', 'initial account amount')
                        ->sum('amount');
                @endphp
                @if ($initialAccountAmount > 0)
                    <p class="indented"><strong>&nbsp;&nbsp;&nbsp;&nbsp; * {{ __('Initial Account Amount') }}:</strong>
                        Bs. {{ number_format($initialAccountAmount, 2) }}</p>
                @endif

                {{-- 3️⃣ Incluir "payment of quota" --}}
                @php
                    $paymentOfQuota = $items
                        ->where('transaction_type_income_expense', 'income')
            ...
            @if ($items->where('transaction_type_income_expense', 'expense')->isNotEmpty())
                ...
                @php
                    $initialAccountAmount = $items
                        ->where('transaction_type_income_expense', 'expense')
