@section('title')
{{__('Uncollectible accounts')}}
@endsection
<div>
    <section class="flex flex-col gap-y-8 py-8 mt-14">
        <x-breadcrumb
            pageTitle="{{__('Uncollectible accounts')}}"
            breadcrumbMainUrl="{{ route('uncollectibleAccounts.index') }}"
            breadcrumbMain="{{__('Uncollectible accounts')}}"
        ...
        >
            ...
            <a href="#" wire:click="openModal"
               class="fi-btn bg-orange-500 text-white hover:bg-custom-500 rounded-lg px-3 py-2 text-sm font-semibold inline-flex items-center shadow-sm transition duration-75">
                {{ __('Create an uncollectible account') }}
            </a>
            @endcan
        </x-breadcrumb>
        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
            ...
            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                ...
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                    ...
                    @can('editar prestamos')
                    <td class="px-6 py-4">
                        <a href="#" wire:click="showRevertModal({{ $loan->id }})"
                           class="font-medium text-blue-600 dark:text-blue-500 hover:underline">
                           {{ __('Revert to a collectible account') }}</a>
                    </td>
                    @endcan
                </tr>
                @empty
                <tr>
                    <td colspan="8" class="px-6 py-4 text-center text-gray-500 dark:text-gray-400">
                        {{__('No uncollectible accounts were found.')}}
                    </td>
                </tr>
                @endforelse
                </tbody>
        ...
        @include('components.components.modalRevertCollective')
        @include('components.components.modalUncollectible')
    </section>
</div>
