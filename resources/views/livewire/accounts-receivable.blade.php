...
<div>
    <section class="flex flex-col gap-y-8 py-8 mt-14">
        <x-breadcrumb pageTitle="{{ __('Accounts receivable') }}"
            breadcrumbMainUrl="{{ route('accountsReceivable.index') }}" breadcrumbMain="{{ __('Accounts receivable') }}"
            breadcrumbCurrent="{{ __('List') }}">
        </x-breadcrumb>
        @include('components.components.messagesFlash')
        @include('components.components.modalCollect')
        <!--Table-->
        <!-- Tabla -->
                        ...
                        <tr
                            ...
                            @can('editar ingresos')
                                <td class="px-6 py-4 action-column">
                                    <a href="#" wire:click="collectModal({{$income->id}})"
                                        class="font-medium text-blue-600 dark:text-blue-500 hover:underline">{{ __('Collect payment') }}</a>
                                </td>
                            @endcan
                        </tr>
                    @empty
