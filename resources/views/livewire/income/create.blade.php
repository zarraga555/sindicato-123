...
<div>
    <section class="flex flex-col gap-y-8 py-8 mt-14">
        <x-breadcrumb pageTitle="Ingreso por Movil" breadcrumbMainUrl="{{ route('income.index') }}"
            breadcrumbMain="Ingresos por Moviles" breadcrumbCurrent="Crear">
            <!-- Mostrar el total en pantalla -->
            <p class="text-2xl font-bold">Total: <span id="total">0.00</span></p>

            <a href="#" wire:click="openModalExpense"
        ...
        @include('components.components.modalExpense')
        <div class="grid flex-1 auto-cols-fr gap-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div>
                        ...
                        <input type="number" id="amount_hoja_semanal" name="amount_hoja_semanal"
                            ...
                            placeholder="Ingresa un monto" required wire:model="amount_hoja_semanal">
                        @error('amount_hoja_semanal')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" id="hoja_semanal_serie" name="hoja_semanal_serie"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                            placeholder="Ingrese el número de serie" required wire:model="hoja_semanal_serie">
                        @error('hoja_semanal_serie')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-4">
                    ...
                    <div>
                        ...
                        <input type="number" id="amount_hoja_domingo" name="amount_hoja_domingo"
                            class="sumar-total mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                            placeholder="Ingresa un monto" required wire:model="amount_hoja_domingo">
                        @error('amount_hoja_domingo')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" id="hoja_domingo_serie" name="hoja_domingo_serie"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                            placeholder="Ingrese el número de serie" required wire:model="hoja_domingo_serie">
                        @error('hoja_domingo_serie')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div>
                        ...
                        <input type="number" id="multas" name="multas"
                            class="sumar-total mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                            placeholder="Ingresa un monto" wire:model="multas">
                        @error('multas')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <!-- Lavado Auto Rojo-->
                    <div>
                        <label for="lavado_auto_rojo"
                            class="block text-sm font-medium text-gray-700">{{ $incomeItems->firstWhere('id', 25)->name ?? 'No encontrado' }}</label>
                        <input type="number" id="lavado_auto_rojo" name="lavado_auto_rojo"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                            placeholder="Ingresa un monto" wire:model="lavado_auto_rojo">
                        @error('lavado_auto_rojo')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <!-- Lavado Auto Azul-->
                    <div>
                        <label for="lavado_auto_azul"
                            class="block text-sm font-medium text-gray-700">{{ $incomeItems->firstWhere('id', 28)->name ?? 'No encontrado' }}</label>
                        <input type="number" id="lavado_auto_azul" name="lavado_auto_azul"
                            class="sumar-total mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                            placeholder="Ingresa un monto" wire:model="lavado_auto_azul">
                        @error('lavado_auto_azul')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <!-- Aporte para Seguro -->
                    <div>
                        <label for="aporte_seguro"
                            class="block text-sm font-medium text-gray-700">{{ $incomeItems->firstWhere('id', 27)->name ?? 'No encontrado' }}</label>
                        <input type="number" id="aporte_seguro" name="aporte_seguro"
                            class="sumar-total mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                            placeholder="Ingresa un monto" wire:model="aporte_seguro">
                        @error('aporte_seguro')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror
                    </div>
            ...
            <div class="fi-form-actions mt-4">
                <div class="fi-ac gap-3 flex flex-wrap items-center justify-between w-full">
                    <div class="flex gap-3">
                        ...
                        form?.addEventListener('form-processing-finished', () => {
                            ...
                            id="save-button" wire:click="save">
                            <!-- Indicador de carga -->
                    ...
                    <div class="flex gap-3">
                        <!-- Botón Pagar con QR y Crear Otro -->
                        <button wire:loading.attr="disabled" wire:click="payWithQRAndCreateAnother"
                            class="fi-btn relative grid-flow-col items-center justify-center font-semibold transition duration-75 rounded-lg px-3 py-2 text-sm inline-grid shadow-sm bg-green-600 text-white hover:bg-green-500 focus-visible:ring-2 focus-visible:ring-green-500/50 dark:bg-green-500 dark:hover:bg-green-400 dark:focus-visible:ring-green-400/50 gap-1.5 fi-ac-action fi-ac-btn-action">
                            <span class="fi-btn-label">{{__('Pay with QR and create another')}}</span>
                        </button>

                        <!-- Botón Guardar Pago Pendiente y Crear Otro -->
                        <button wire:loading.attr="disabled" wire:click="savePendingPaymentAndCreateAnother"
                            class="fi-btn relative grid-flow-col items-center justify-center font-semibold transition duration-75 rounded-lg px-3 py-2 text-sm inline-grid shadow-sm bg-yellow-600 text-white hover:bg-yellow-500 focus-visible:ring-2 focus-visible:ring-yellow-500/50 dark:bg-yellow-500 dark:hover:bg-yellow-400 dark:focus-visible:ring-yellow-400/50 gap-1.5 fi-ac-action fi-ac-btn-action">
                            <span class="fi-btn-label">{{__('Save pending payment and create another one')}}</span>
                        </button>
                    </div>
                </div>
            </div>
