@section('title')
    {{ __('Collection of Dues') }}
@endsection
<div>
    <section class="flex flex-col gap-y-8 py-8 mt-14">
        <x-breadcrumb pageTitle="{{ __('Collection of Dues') }}"
            breadcrumbMainUrl="{{ route('uncollectibleAccounts.index') }}" breadcrumbMain="{{ __('Collection of Dues') }}"
                ...
                <tbody>
                    @forelse($loans as $loan)
                        <tr
                            ...
                            @can('editar prestamos')
                                <td class="px-6 py-4">
                                    <a href="#" wire:click="openModalPayment({{$loan->id}})" wire:loading.attr="disabled"
