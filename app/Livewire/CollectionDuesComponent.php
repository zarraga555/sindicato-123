...
class CollectionDuesComponent extends Component
{
    use WithPagination;
    ...
    public function render()
    {
        try {
            ...
            return view('livewire.collection-dues-component', compact('loans'));
        } catch (\Exception $e) {
    ...
    public function reedLoan($id)
    {
        try {
            // Buscar el préstamo, si no existe lanza error automáticamente
            $loanDetails = Loans::findOrFail($id);

            // Obtener la cuota más reciente que aún tenga saldo pendiente ("unpaid" o "partial")
            $fee = paymentPlans::where('loan_id', $id)
            ...
            $this->debtorName = $loanDetails->driver_partner_name;
            $this->currency = optional($loanDetails->cashFlows->banks)->currency_type ?? 'N/A'; // Evita error si no hay relación
            ...
            // Manejo de errores
            Log::error('Error en reedLoan: ' . $e->getMessage());
    ...
    public function paymentDues()
    {
        ...
        try {
            // Validar el préstamo
            $loan = Loans::find($this->loanId);
            if (!$loan) {
                session()->flash('error', 'El préstamo no existe.');
                return;
            }
            ...
            $fees = paymentPlans::where('loan_id', $this->loanId)
                ->whereIn('paymentStatus', ['unpaid', 'partial'])
                ->orderBy('datePayment', 'asc')
                ->get();
            ...
            $pendingFees = paymentPlans::where('loan_id', $this->loanId)
                ->whereIn('paymentStatus', ['unpaid', 'partial'])
                ->exists();
    
            if (!$pendingFees) {
                $loan->update(['status' => 'paid']);
            } elseif ($loan->status === 'unpaid' && $atLeastOnePaid) {
    ...
    {
        try {
            if (!$this->loanId || !$this->amount_paid || $this->amount_paid <= 0) {
            ...
            $cashFlow = CashFlow::create([
                ...
                'detail' => "Pago de Cuota: {$accountLetter->currency_type} {$this->amount_paid}, Movil: " . optional($loan)->vehicle_id,
                'type_transaction' => 'Payment of dues',
